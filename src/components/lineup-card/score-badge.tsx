...
export function getScoreBadgeColor(
  ...
  const s = scoreStatus?.toUpperCase() ?? "";
  const isPlaying = s === "PLAYING" || s === "STARTED";
  if (isPlaying) return "bg-green-500 text-white";
  if (score >= 60) return "bg-green-600 text-white";
  if (score >= 40) return "bg-yellow-500 text-zinc-900";
...
export function ScoreBadge({
  ...
  return (
    <span
      className={cn(
        "font-bold rounded-full text-center tabular-nums",
        size === "sm"
          ? "text-[9px] px-1.5 py-px min-w-[20px]"
          : "text-[10px] px-1.5 py-px min-w-[24px]",
