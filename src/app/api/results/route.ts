...
export async function GET(request: Request) {
  ...
  try {
    ...
    const fixtureSlug = rows[0].fixtureSlug;

    const leaderboards: LeaderboardSummary[] = rows.map((row) => {
      const rankings: any[] = JSON.parse(row.rankingsJson);
      const parsed = rankings.map(parseRanking);

      const podium = parsed.slice(0, 3);
      const clanEntries = parsed.filter((r) => clanSlugs.has(r.user.slug));
    ...
    return NextResponse.json({ gameWeek, fixtureSlug, leaderboards });
  } catch (error) {
